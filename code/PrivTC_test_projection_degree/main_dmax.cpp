...
int main()
{
    ...
    for (double d_max_rate = 1.0; d_max_rate >= 0.19; d_max_rate -= 0.2)
    {
        double epsilon = 2.0;
        double CDP_avg_time = 0.0;
        double c_var = 0.0, f_var = 0.0;
        double c_relative_error = 0.0, f_relative_error = 0.0;
        int projection_d_max = (int)(d_max * d_max_rate);
        for (int j = 0; j < 10; j++)
        {
            auto start = std::chrono::steady_clock::now();
            std::pair<double, double> CDP_answer = centralized_DP(neighbors, projection_d_max, epsilon);
        ...
        std::cout << "time:" << "projectiondmax:" << projection_d_max << " " << d_max <<  " " << "CDP_avg_time:" << CDP_avg_time << std::endl;
    ...
    for (double d_max_rate = 1.0; d_max_rate >= 0.19; d_max_rate -= 0.2)
    {
        ...
        {
            auto start = std::chrono::steady_clock::now();
            std::pair<double, double> LDP_answer = localized_DP(neighbors, projection_d_max, epsilon1, epsilon2);
            auto end = std::chrono::steady_clock::now();
        ...
        c_relative_error /= 10;
        f_relative_error /= 10;
        std::cout << "epsilon:" << epsilon << "epsilon1:" << epsilon1 << "epsilon2:" << epsilon2 << std::endl;
        std::cout << "time:" << "projectiondmax:" << projection_d_max << " " << d_max << " " << "LDP_avg_time:" << LDP_avg_time << std::endl;
        std::cout << "var:" << c_var << " " << f_var << std::endl;
